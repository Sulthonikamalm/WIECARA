...
(function () {
    ...
    const CONFIG = {
      ...
      API: {
        BASE_PATH: '../api',
        ENDPOINTS: {
          STATISTICS: '/cases/get_public_stats.php',
    ...
    async function fetchStatistics() {
      try {
        const res = await fetch('../api/cases/get_public_stats.php');
        const result = await res.json();
        
        if (result.status === 'success') {
          statisticsData = {
            total_cases: result.data.total_cases || 0,
            cases_completed: result.data.resolved_cases || 0
          };
        } else {
        ...
        // Fallback: try setting to 0
        statisticsData = { total_cases: 0, cases_completed: 0 };
        setupCounterObserver();
      }
    }
    ...
    if (checkBtn && progressStatusDiv && kodeInput) {
      checkBtn.addEventListener('click', function () {
        ...
        .then(data => {
          ...
          if (data.status === 'success' && data.data && data.data.laporan) {
            ...
            const safeKode = encodeURIComponent(
              sanitizeInput(data.data.laporan.kode_pelaporan || '')
            );
...
// ============================================
// KABAR WIECARA / WAWASAN CAROUSEL
// ============================================
...
(function initKabarWiecaraCarousel() {
  ...
  document.addEventListener('keydown', (e) => {
    const section = document.getElementById('kabarWiecara');
    if (!section) return;
  ...
  logger.log('✅ Kabar Wiecara Carousel - Swipe Gesture Ready');
})();
