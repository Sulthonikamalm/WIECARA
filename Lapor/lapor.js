...
(function () {
    ...
    const totalSteps = 7;
    const formData = {};
    let isEmergencyRedirecting = false;
    ...
    function addConfidenceIndicator(element, score) {
        ...
        if (score >= 0.8) {
            indicator.innerHTML = 'âœ“ <span>Tinggi</span>';
            indicator.classList.add('high');
        } else if (score >= 0.5) {
            indicator.innerHTML = 'âš ï¸ <span>Sedang</span>';
            indicator.classList.add('medium');
        } else {
            indicator.innerHTML = 'â“ <span>Rendah</span>';
    ...
    function showAutoFillNotification(confidenceScores) {
        ...
        notification.innerHTML = `
            ...
            <p>Data dari percakapanmu dengan ARA sudah diisi. Silakan periksa dan ubah jika perlu.</p>
    ...
    async function processStep1() {
        if (isEmergencyRedirecting) return;

        if (formData.statusDarurat === 'darurat') {
            await dispatchEmergencyAndRedirect();
        } else if (formData.statusDarurat === 'tidak') {
            goToStep(2);
        }
    }

    function buildEmergencyWhatsAppUrl() {
        const phoneNumber = '6281234567890';
        const message = encodeURIComponent('DARURAT! Saya membutuhkan bantuan segera dari Satgas PPKPT.');
        return `https://wa.me/${phoneNumber}?text=${message}`;
    }

    function getEmergencyLocationForDispatch() {
    ...
    async function dispatchEmergencyAndRedirect() {
        isEmergencyRedirecting = true;

        try {
            const emergencyLocation = await getEmergencyLocationForDispatch();
            await fetch('../api/emergency/create_case.php', {
                method: 'POST',
                ...
                body: JSON.stringify({
                    ...
                    fresh_location: emergencyLocation
                })
            });
        } catch (error) {
            // WhatsApp tetap dibuka sebagai fallback walaupun dashboard gagal menerima sinyal.
        } finally {
            window.location.href = buildEmergencyWhatsAppUrl();
        }
    }
    ...
    function showSuccessModal(kodeLaporan) {
        ...
        document.getElementById('btnCloseModal').addEventListener('click', function () {
            closeSuccessModal();
            setTimeout(() => {
                window.location.href = `../LandingPage/Landing_Page.html?submitted=true&kode=${kodeLaporan}`;
            }, 300);
        });
    }
    ...
    function showValidationErrors(errors) {
        let errorMessage = 'âŒ Validasi Gagal:\n\n';
    ...
    function showErrorModal(errorMessage) {
        alert(`âŒ Gagal Mengirim Laporan!\n\n${errorMessage}\n\nSilakan coba lagi atau hubungi admin jika masalah berlanjut.`);
    }
    ...
    function initVoiceInput() {
        ...
        recognition.onerror = function (event) {
            ...
            switch (event.error) {
                ...
                case 'not-allowed':
                    showVoiceError('Izin mikrofon ditolak. Klik ikon ðŸ”’ di address bar untuk mengizinkan.');
                    shouldKeepRunning = false;
                    break;
    ...
    function finishRecording(btnVoiceInput, voiceRecordingIndicator, detailKejadian) {
        ...
        if (liveTranscriptText && finalTranscript.trim()) {
            liveTranscriptText.innerHTML = '<span style="color: #10b981;">âœ“ Selesai</span>';
            setTimeout(() => {
                if (liveTranscriptPreview) {
    ...
    async function startRecording() {
        ...
        try {
            ...
            if (error.name === 'NotAllowedError' || error.name === 'PermissionDeniedError') {
                showVoiceError('Izin mikrofon ditolak. Klik ikon ðŸ”’ di address bar browser untuk mengizinkan.');
    ...
    function handleEmotionDetected(emotionData) {
        ...
        const emotionMap = {
            sedih: { icon: 'ðŸ˜¢', text: 'Saya merasakan kesedihan dalam ceritamu', class: 'sedih' },
            marah: { icon: 'ðŸ˜ ', text: 'Saya merasakan kemarahan di sini', class: 'marah' },
            takut: { icon: 'ðŸ˜°', text: 'Kamu mungkin merasa takut atau cemas', class: 'takut' },
            putusAsa: { icon: 'ðŸ’”', text: 'Saya di sini untukmu. Kamu tidak sendiri.', class: 'putus-asa' },
            malu: { icon: 'ðŸ˜¶', text: 'Tidak apa-apa merasa malu, itu wajar', class: 'sedih' },
            bingung: { icon: 'ðŸ¤”', text: 'Saya mengerti kebingunganmu', class: 'sedih' },
            lega: { icon: 'ðŸ˜Œ', text: 'Senang kamu merasa lebih baik', class: 'lega' },
            berharap: { icon: 'ðŸ™', text: 'Terima kasih sudah berbagi harapanmu', class: 'lega' }
        };

        const emotion = emotionMap[emotionData.primary] || { icon: 'ðŸ’™', text: 'Saya mendengarkanmu', class: '' };
    ...
    function handleAudioEvent(eventData) {
        ...
        const eventMap = {
            crying: { icon: 'ðŸ˜¢', text: 'Tidak apa-apa menangis. Saya di sini.', class: 'crying' },
            sobbing: { icon: 'ðŸ˜¢', text: 'Saya merasakan kesedihanmu. Ceritakan saja.', class: 'crying' },
            laughing: { icon: 'ðŸ˜Š', text: '', class: 'laughing' }, // No need to show for laughing
            scream: { icon: 'âš ï¸', text: 'Apakah kamu baik-baik saja?', class: 'scream' },
            distress: { icon: 'ðŸ’™', text: 'Saya merasakan kamu sedang dalam kesulitan', class: 'distress' }
        };
